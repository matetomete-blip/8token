...
const adminHtmlPath = '/opt/8token/admin.html';

// === PART 1: Fix server.js ===
let code = fs.readFileSync(serverPath, 'utf8');
console.log('[SERVER] Original lines:', code.split('\n').length);

// 1a. Add express-session import if missing
if (!code.includes("require('express-session')")) {
  code = code.replace(
    "const rateLimit = require('express-rate-limit');",
    "const rateLimit = require('express-rate-limit');\nconst session = require('express-session');"
  );
  console.log('[SERVER] Added express-session import');
}

// 1b. Add ADMIN_TOTP_ACCESS_SECRET env var if missing
if (!code.includes('ADMIN_TOTP_ACCESS_SECRET')) {
  ...
  console.log('[SERVER] Added ADMIN_TOTP_ACCESS_SECRET');
}

// 1c. Add session middleware after express.json if missing
if (!code.includes('session({')) {
  code = code.replace(
    "app.use(express.json({ limit: '10mb' }));",
    `app.use(express.json({ limit: '10mb' }));
app.use(session({
  secret: process.env.JWT_SECRET || '8token-session-secret',
  resave: false,
  ...
  console.log('[SERVER] Added session middleware');
}

// 1d. Add login endpoints after adminAuth function if missing
...
app.post('/api/admin/login', async (req, res) => {
  try {
    ...
    const result = totpVerify.verify({ token: totpToken, secret: ADMIN_TOTP_ACCESS_SECRET });
    if (!result) return res.status(403).json({ error: 'Código 2FA inválido ou expirado' });
...
`;
    ...
    console.log('[SERVER] Added login endpoints');
  } else {
    console.error('[SERVER] FAILED: Could not find adminAuth function');
    process.exit(1);
  }
}
...
// 2a. Replace getAdminSecret with session-aware version
...
function getAdminSecret() {
  ...
  console.log('[HTML] Replaced getAdminSecret with session check');
}

// 2b. Replace modal text and input for TOTP
...
console.log('[HTML] Updated modal text and input');

// 2c. Replace submitAdminLogin with TOTP API call
...
if (oldSubmit && !html.includes('async function submitAdminLogin')) {
  ...
  console.log('[HTML] Replaced submitAdminLogin with TOTP API call');
}

// 2d. Update apiFetch to use credentials
...
if (oldApi && !html.includes("credentials: 'same-origin'")) {
  ...
  console.log('[HTML] Updated apiFetch with credentials');
}

// 2e. Add initAdminAuth if not present
...
`;
  ...
  if (scriptEnd > 0) {
    ...
    console.log('[HTML] Added initAdminAuth');
  }
}
