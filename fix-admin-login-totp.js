...
// 1. Replace getAdminSecret to check session first, then show TOTP login modal
...
const newGetAdminSecret = `async function checkAdminSession() {
        ...
        function getAdminSecret() {
            // For backward compat with API calls that still use header
...
if (html.includes(oldGetAdminSecret)) {
    ...
    console.log('[1] WARNING: Could not find exact getAdminSecret pattern');
}

// 2. Replace showAdminLoginModal to use TOTP instead of password
...
if (html.includes(oldShowModal)) {
    ...
    console.log('[2] WARNING: Could not find exact showAdminLoginModal pattern');
}
...
if (html.includes(oldSubmitLogin)) {
    ...
    console.log('[3] WARNING: Could not find exact submitAdminLogin pattern');
}

// 4. Update apiFetch to not require ADMIN_SECRET header when session is active
...
const newApiFetch = `async function apiFetch(url, options = {}) {
            ...
            // Include ADMIN_SECRET as fallback for backward compat
...
if (html.includes(oldApiFetch)) {
    ...
    console.log('[4] Updated apiFetch to use session cookies and handle 401/403');
} else {
    console.log('[4] WARNING: Could not find exact apiFetch pattern');
}

// 5. Add initialization code to check session on page load
const initCode = `
...
// Insert before the closing </script> tag or after existing init code
...
console.log('Final size:', html.length, 'bytes');