...
(async () => {
  // Step 1: Set to mensal first
  console.log("--- Setting to mensal ---");
  const r1 = await putPlan("mensal");
  ...
  console.log("USER after PUT mensal:", JSON.stringify(u1));

  // Step 2: Change to trimestral
  console.log("--- Changing to trimestral ---");
  const r2 = await putPlan("trimestral");
  ...
  console.log("USER after PUT trimestral:", JSON.stringify(u2));

  // Step 3: Change back to mensal
  console.log("--- Changing back to mensal ---");
  const r3 = await putPlan("mensal");
  console.log("PUT result:", JSON.stringify(r3));
  await new Promise(r => setTimeout(r, 1500));
  const { data: u3 } = await supabase.from("users").select("id, email, plan").eq("email", "teste@teste.com").single();
  console.log("USER after PUT mensal:", JSON.stringify(u3));
